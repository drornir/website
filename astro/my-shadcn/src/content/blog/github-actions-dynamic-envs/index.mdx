...
function jqEquivalent(vars: Record<string, string>, envFilter: RegExp, clusterFilter: RegExp, namespaceFilter: RegExp): string {
  ...
  const filteredEnvs = terraformCreatedVars
    .filter((env: Env) => envFilter.test(env.env))
    .filter((env: Env) => clusterFilter.test(env.cluster_name))
