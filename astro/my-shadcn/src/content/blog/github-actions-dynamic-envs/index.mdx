...
The idea is the inside my terraform module that creates the environment, I also create an
...
> Gotcha: Github will uppercase the variable*name, so know that if you set e.g `terraform_env*...`it will become`TERRAFORM*ENV*...`. This is important to keep in mind when referencing the variable later in github.
